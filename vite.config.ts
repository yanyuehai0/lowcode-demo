...
const externalizeGlobalsPlugin = (): Plugin => {
     ...
     */
    const parseNamedImports = (importsStr: string, globalName: string): string => {
        return importsStr
            .split(',')
            .map((item) => item.trim())
            .filter(Boolean)
    ...
    return {
        ...
        transform(code) {
            ...
            for (const [pkg, globalName] of Object.entries(globals)) {
                ...
                const defaultImportRegex = new RegExp(`import\\s+(\\w+)\\s+from\\s*['"]${pkg}([^'"]*)['"]`, 'g');
                modified = modified.replace(defaultImportRegex, `const $1 = window.${globalName}`);
                ...
                modified = modified.replace(namespaceImportRegex, `const $1 = window.${globalName};`);
